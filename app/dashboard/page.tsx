...
import { Search, Plus, Edit3, Grid, List, Settings, X, Eye, Info, Users, User } from "lucide-react"
...
export default function PokemonCollectionManager() {
  ...
  const [editingCard, setEditingCard] = useState<{ cardId: number; variant: string; entryId: string } | null>(null)
  ...
  const addCardEntry = (cardId: number, variant: string, condition: string, language: string) => {
  ...
  const updateCardEntry = (cardId: number, variant: string, entryId: string, updates: Partial<CardEntry>) => {
    setCollection((prev) => {
  ...
  const removeCardEntry = (cardId: number, variant: string, entryId: string) => {
    setCollection((prev) => {
  ...
  const startEditingCard = (cardId: number, variant: string, entryId: string) => {
        ...
        <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <div className="flex items-center gap-2 mr-4">
                    <div className="flex items-center gap-1">
                      <Button
                        variant={addingMode === "individual" ? "default" : "outline"}
                        size="sm"
                        onClick={() => {
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs">
                          <div className="space-y-2 text-sm">
                            <div>
                              <strong>Individual Mode:</strong> Add cards one by one using the + dropdown on each card.
                              Perfect for adding specific cards with different conditions.
                            </div>
                            <div>
                              <strong>Bulk Mode:</strong> Select multiple cards with checkboxes, then add the same
                              variant/condition/language to all at once. Great for adding many similar cards quickly.
                            </div>
                          </div>
                        </TooltipContent>
                      </Tooltip>
              ...
              {addingMode === "bulk" && selectedCards.size === 0 && (
                <div className="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="text-sm text-gray-600 flex items-center gap-2">
                    ...
                    <span>Select cards using the checkboxes to add them in bulk with the same settings.</span>
                  </div>
                </div>
              )}
          ...
          >
            {filteredCards.map((card) => (
              <Card
                key={card.id}
                className={`transition-all ${selectedCards.has(card.id) ? "ring-2 ring-blue-500" : ""} ${viewMode === "list" ? "p-2" : ""}`}
              >
                          ...
                          return (
                            <div key={variant} className="space-y-1">
                              <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                                ...
                                <div className="flex items-center gap-2">
                                  <span className="text-sm text-gray-600">Total: {totalQuantity}</span>
                                      ...
                                      <DropdownMenuContent className="w-80 p-3">
                                        <div className="space-y-3">
                                          ...
                                          <div className="border-t pt-3">
                                            <div className="text-xs text-gray-600 mb-2">Select Language:</div>
                                              ...
                                              {cardLanguages.slice(0, 10).map((language) => (
                                                ...
                                                >
                                                  <input
                                                    ...
                                                    name="language"
                                                    value={language.code}
                                                    checked={selectedQuickLanguage === language.code}
                                                    onChange={() => setSelectedQuickLanguage(language.code)}
                                                    className="sr-only"
                                                  />
                                                  {language.flag}
                                                </label>
                                            ...
                                            <div className="text-xs text-gray-600 mb-2">Add Card with Condition:</div>
                                    ...
                                    <div className="flex items-center gap-1 ml-auto">
                                      ...
                                      <Dialog>
                                        <DialogTrigger asChild>
                                          ...
                                          >
                                            <Edit3
                                              className={`h-3 w-3 ${entry.note || entry.photos.length > 0 ? "text-blue-600" : "text-gray-400"}`}
                                            />
                                          </Button>
                                        </DialogTrigger>
